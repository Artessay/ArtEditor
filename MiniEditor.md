...
​		采用临时文本存储的方法，对用户所编辑的文档进行临时存储，有助于我们对于用户的保存文档并退出以及不保存等操作的实现
如果用户在退出编辑器的时候选择了保存并退出，那么用户所编辑的临时文件会覆盖掉原来的文件，实现保存的功能，如果用户，在退出的时候选择了不保存，直接退出，该临时文件咋不会覆盖用户原本编辑的文件？用户原来的文档得以安全保存。

​		通过临时文件编辑存储的方法，可以在软件层面提高用户文档编辑的安全性。这样即使用户在编辑的过程中遇见了系统临时关机等情况，用户所编辑的文档也不会丢失。

在用户退出编辑器之后，无论用户是否选择了保存文件，该领取文件都会被删除
